...
For instance, to use self-signed certificates created with [mkcert](https://github.com/FiloSottile/mkcert) do as
follows:
...
4. Add these lines to the `./docker-compose.override.yml` file about `CADDY_EXTRA_CONFIG` environment and volume for
   the `php` service :
    ```diff
    php:
      environment:
